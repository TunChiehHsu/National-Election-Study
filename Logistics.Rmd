---
title: "HW3 [Team 2]"
author: "Jialei Guo, Ziying Sun, Tun-Chieh Hsu, Xialingzi Jin"
...
3. Obtain a point estimate and create a 95% confidence interval for the odds ratio for voting Republican for a rich person (income category 5) compared to a poor person (income category 1). *Hint this is more than a one unit change; calculate manually and then show how to modify the output from confint*. 
...
```{r}
ggplot(res, aes(x = year, y = coefficient)) +
  geom_point() + 
  geom_errorbar(aes(ymax = `1sd upper bound`, ymin=`1sd lower bound`)) +
  theme_bw() +
  theme(
    plot.background = element_blank()
   ,panel.grid.major = element_blank()
...
Several reasons can lead to the missing values in the dataset. If the data is manually digitalized
